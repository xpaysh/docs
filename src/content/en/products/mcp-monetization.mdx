...
Use the **[Monetize Wizard](https://www.xpay.sh/monetize-mcp-server/)** to register your server in under 2 minutes — no account needed to start:

1. Paste your MCP server URL (SSE or Streamable HTTP)
2. Set per-tool pricing (the wizard auto-discovers your tools)
3. Sign in and provide your USDC wallet address on Base
4. Get your proxy URL

Alternatively, if you already have an account, go to **[Dashboard → MCP Servers → Add MCP Server](https://app.xpay.sh/dashboard/charge-per-use/mcp-servers/)** for the full setup experience. Make sure to configure your wallet and portal branding under **[Account Config](https://app.xpay.sh/dashboard/charge-per-use/account-config/)** first.
