...
class MultiTenantAgentManager {
  private firewalls: Map<string, XpayFirewall> = new Map()
  ...
  async initializeTenant(tenantId: string, tier: string = 'basic') {
    ...
    const firewall = new XpayFirewall({
      endpoint: `https://firewall-${tenantId}.xpay.sh`,
    ...
    this.firewalls.set(tenantId, firewall)
    return firewall
  }
  ...
  async createAgentForTenant(
    ...
    const firewall = this.getFirewallForTenant(tenantId)
  ...
  async getAgentsForTenant(tenantId: string, userId?: string): Promise<Agent[]> {
    ...
    const agents = await firewall.listAgents({
  ...
  private getFirewallForTenant(tenantId: string): XpayFirewall {
    const firewall = this.firewalls.get(tenantId)
    if (!firewall) {
...
class XpayBatchProcessor implements BatchProcessor {
  ...
  private async processSingleBatch(
    ...
    try {
      ...
      const batchResults = await this.firewall.batchRequests(batchRequest)
...
// Usage example
const batchProcessor = new XpayBatchProcessor(firewall, {
  retryAttempts: 3,
  retryDelay: 1000,
...
class XpayAnalyticsEngine {
  ...
  constructor(
    ...
    this.metricsCollector = new MetricsCollector(firewall)
...
// Usage example
const analytics = new XpayAnalyticsEngine(firewall, {
...
describe('Xpay Integration Tests', () => {
  let firewall: XpayFirewall
  let testAgent: Agent
  
  beforeEach(async () => {
    // Initialize test firewall
    firewall = new XpayFirewall({
    ...
    // Create test agent
    testAgent = await firewall.createAgent({
  ...
  afterEach(async () => {
    // Cleanup
    if (testAgent) {
      await firewall.deleteAgent(testAgent.id)
    }
  })
  ...
  describe('Agent Lifecycle', () => {
    ...
    it('should update agent limits', async () => {
      const updated = await firewall.updateAgent(testAgent.id, {
        dailyLimit: 20,
        perCallLimit: 2
      })
    ...
    it('should pause and resume agent', async () => {
      await firewall.pauseAgent(testAgent.id)
      let agent = await firewall.getAgent(testAgent.id)
      expect(agent.status).toBe('paused')
      
      await firewall.resumeAgent(testAgent.id)
      agent = await firewall.getAgent(testAgent.id)
  ...
  describe('Payment Processing', () => {
    it('should process successful payment', async () => {
      const response = await firewall.protectedFetch('https://api.example.com/test', {
      ...
      const updatedAgent = await firewall.getAgent(testAgent.id)
    ...
    it('should reject payment when limit exceeded', async () => {
      // Set very low limit
      await firewall.updateAgent(testAgent.id, { perCallLimit: 0.001 })
      
      await expect(
        firewall.protectedFetch('https://api.example.com/expensive', {
          agentId: testAgent.id,
          method: 'POST'
        })
    ...
    it('should handle batch requests correctly', async () => {
      ...
      const results = await firewall.batchRequests(requests)
  ...
  describe('Error Handling', () => {
    it('should handle network timeouts gracefully', async () => {
      ...
      await expect(
        firewall.protectedFetch('https://httpstat.us/200?sleep=8000', {
          agentId: testAgent.id,
          timeout: 2000
        })
    ...
    it('should retry failed requests', async () => {
      const retryFirewall = new XpayFirewall({
        endpoint: process.env.XPAY_TEST_ENDPOINT!,
        apiKey: process.env.XPAY_TEST_API_KEY!,
        retries: 3,
        retryDelay: 100
      ...
      const response = await retryFirewall.protectedFetch('https://api.example.com/retry', {
        agentId: testAgent.id
      })
  ...
  describe('Analytics and Monitoring', () => {
    it('should track spending analytics correctly', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await firewall.protectedFetch('https://api.example.com/analytics', {
          agentId: testAgent.id
        })
      }
    ...
    it('should retrieve transaction history', async () => {
      ...
      await firewall.protectedFetch('https://api.example.com/history', {
        agentId: testAgent.id
      })
      
      const history = await firewall.getTransactionHistory({
        agentIds: [testAgent.id],
        limit: 10
      })
...
export class LoadTestRunner {
  constructor(private firewall: XpayFirewall) {}
  ...
  async runLoadTest(config: LoadTestConfig): Promise<LoadTestResults> {
    ...
    try {
        ...
        promises.push(
          new Promise(resolve => {
            setTimeout(async () => {
              ...
              try {
                await this.firewall.protectedFetch(config.targetUrl, {
                  agentId: agent.id
                })
                resolve({
                  success: true,
      ...
      // Cleanup test agents
      await Promise.all(
        agents.map(agent => this.firewall.deleteAgent(agent.id))
      )
    }
