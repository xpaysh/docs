...
- ✅ Plugin [installed and activated](/merchants/woocommerce/installing)
- ✅ A WooCommerce REST API key with **Read** scope ([how to generate](/merchants/woocommerce/rest-api-keys))
...
From **0.7.0** the plugin adds an **xpay✦ Commerce** menu in the WordPress sidebar, separate from the
settings page you just used:
...
<Callout type="info">
  **Settings has not moved.** Connecting, disconnecting and every option stay at **Settings → xpay**,
  exactly as before — the new menu links to it. Existing bookmarks keep working.
</Callout>

The Dashboard, AI Revenue and Product Feed screens read your own WordPress database and work
**without connecting**, so you can see AI crawler traffic before deciding to connect anything.
...
- → [Privacy & telemetry](/merchants/woocommerce/privacy-telemetry) — opt in / opt out of anonymous lifecycle events
- → [Audit readiness checklist](/merchants/woocommerce/audit-readiness) — what each of the 8 ✓ rows means
- → [Troubleshooting](/merchants/woocommerce/troubleshooting) — if something stayed yellow
